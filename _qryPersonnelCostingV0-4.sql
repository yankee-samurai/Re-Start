...
StdDevSet AS
	(SELECT
		...
		--,STDEV(Salary) OVER (PARTITION BY EmpNo) AS SySal
		,Salary AS ySal
		,FICA AS yFICA
		,PERS AS yPERS
		,Ins AS yIns
		,AVG(FY) OVER (PARTITION BY EmpNo) AS xbar
		--,STDEV(FY) OVER (PARTITION BY EmpNo) AS Sx
		,FY AS x
	FROM Y
...
CorrelSet AS
	(SELECT
		...
		,SUM((x - xbar) * (x - xbar)) OVER (PARTITION BY Empno) AS B1SalD
	
		--,CASE WHEN STDEVP(FY) + STDEVP(Salary) != 0
		--	THEN (AVG(FY * Salary) - (AVG(FY) - AVG(Salary))) / (STDEVP(FY) + STDEVP(Salary)) 
		--	ELSE 0
		--END AS Rsal
	FROM StdDevSet
	GROUP BY EmpNo, x, ySal, xbar, YbarSal
	)
...
INNER JOIN CorrelSet 
ON StdDevSet.EmpNo = CorrelSet.EmpNo
...
,CorrelSet.B1SalD
--,Sx, n
, ybarsal,  ybarFICA, YbarPERS, ybarIns, ySal, yFICA, yPERS, yIns, xbar,  x



ORDER BY StdDevSet.EmpNo
